...
def extract_invoice_data(file_bytes, file_name):
    ...
    try:
        ...
        st.error(f"Gemini API Error for {file_name}: {e}")
        return None
...
st.markdown("Upload your PDF invoices below. Gemini 2.5 will extract the date and total amounts and output a single Excel file.")

uploaded_files = st.file_uploader(
    "Upload PDF or image invoices (e.g., PDF, JPG, PNG)",
...
if process_button and uploaded_files:
    if not st.secrets["GEMINI_API_KEY"]:
        st.error("The **GEMINI_API_KEY** secret is not configured. Please follow the setup instructions.")
    else:
        ...
        if st.session_state.results:
            ...
            df = df[['File_Name', 'Invoice_Date', 'Total_Amount_Excl_VAT', 'VAT_Amount', 'Total_Amount_Incl_VAT']]
            ...
            st.warning("No data was successfully extracted. Check the error messages above.")
